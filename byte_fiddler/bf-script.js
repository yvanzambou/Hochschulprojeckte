...
const schalter_bit8 = document.querySelector(".schalter-8");
const schalter_bit7 = document.querySelector(".schalter-7");
const schalter_bit6 = document.querySelector(".schalter-6");
const schalter_bit5 = document.querySelector(".schalter-5");
const schalter_bit4 = document.querySelector(".schalter-4");
const schalter_bit3 = document.querySelector(".schalter-3");
const schalter_bit2 = document.querySelector(".schalter-2");
const schalter_bit1 = document.querySelector(".schalter-1");
...
dark.addEventListener('click', () => {
	if (dark_btn.style.left == "0px") { /* hell */
		dark_btn.style.left = "85px";
		document.body.style.backgroundColor = '#e9e9e9';
		tacho.style.backgroundColor = '#d3d3d3';
		tabs.forEach((e) => { e.style.backgroundColor ='rgb(200, 200, 200)'; });
		tds.forEach((e) => { e.style.color ='#000'; });
		document.querySelectorAll("td").forEach((e) => { e.style.color ='#000' });
		tacho_striche.forEach((e) => { e.style.backgroundColor = 'black'; });
		tacho_ziffer.forEach((e) => { e.style.color = 'black'; });
		document.querySelector(".percentage-screen").style.backgroundColor = '#17bb00';
	} else { /* dunkel */
		dark_btn.style.left = "0px";
		document.body.style.backgroundColor = '#232525';
		tacho.style.backgroundColor = '#171717';
		tabs.forEach((e) => { e.style.backgroundColor ='#000'; });
		tds.forEach((e) => { e.style.color ='#fff'; });
		document.querySelectorAll("td").forEach((e) => { e.style.color ='#fff' });
		tacho_striche.forEach((e) => { e.style.backgroundColor = '#fff'; });
		tacho_ziffer.forEach((e) => { e.style.color = '#fff'; });
		document.querySelector(".percentage-screen").style.backgroundColor = '#c7ff00';
	}
});
...
function start() {
	...
	tds[0].textContent = decimalSumme;
	tds[1].textContent = "0x" + decimalSumme.toString(16);
	tds[2].textContent = decToISOChar(decimalSumme);
...
function finaleBoard() {
	for (let i = 0; i <= 255; i++) {
		isoTabelle.innerHTML +=
		`<tr>
			<td class="board-right-body">`+ isoChar(i) + `</td>
			<td class="board-right-body">`+ i.toString(2) + `</td>
			<td class="board-right-body">`+ i + `</td>
			<td class="board-right-body">`+ i.toString(16) + `</td>
			<td class="board-right-body">`+ zeichenName(i) + `</td>
		</tr>`;
	}
}

// Funktionsaufrufe nach dem Start der App
finaleBoard();
start();